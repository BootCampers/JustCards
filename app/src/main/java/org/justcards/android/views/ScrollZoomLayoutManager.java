...
public class ScrollZoomLayoutManager extends RecyclerView.LayoutManager {
    ...
    private float maxScale; //max scale rate default is 1.2f
    ...
    private SparseIntArray itemsX = new SparseIntArray();

    public ScrollZoomLayoutManager(Context context, int itemSpace) {
        this.context = context;
        offsetScroll = 0;
        this.itemSpace = px(itemSpace);
        maxScale = SCALE_RATE;
    }
    ...
    private void layoutItems(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (itemsX.get(position) - offsetScroll + startLeft > getHorizontalSpace() || itemsX.get(position) - offsetScroll + startLeft < -mDecoratedChildWidth - getPaddingLeft()) {
        ...
        for (int i = 0; i < getItemCount(); i++) {
            if (itemsX.get(i) - offsetScroll + startLeft <= getHorizontalSpace() && itemsX.get(i) - offsetScroll + startLeft >= -mDecoratedChildWidth - getPaddingLeft()) {
                if (!itemAttached.get(i)) {
                    ...
                    scrap.setRotation(0);
                    layoutDecorated(scrap, startLeft + x, startTop, startLeft + x + mDecoratedChildWidth, startTop + mDecoratedChildHeight);
    ...
    public int scrollHorizontallyBy(int dx, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        // increase the offset x so when re-layout it can recycle the right views
    ...
    /**
     * @param x start position of the view you want scale
    ...
    public ScrollZoomLayoutManager setContentOffsetY(int contentOffsetY) {
        this.contentOffsetY = contentOffsetY;
        return this;
    }

    public ScrollZoomLayoutManager setMaxScale(float maxScale) {
